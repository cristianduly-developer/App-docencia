...
// ── Colores disponibles para escuelas ─────────────────────────
const COLORES = ["#2D6A4F","#1d4ed8","#7c3aed","#dc2626","#d97706","#0891b2","#be185d","#15803d"];

// ── Ficha escuela ─────────────────────────────────────────────
function FichaEscuela({ escuela, alumnos, docentes, onEditar, onBack }) {
    ...
    <div>
      ...
      <div style={{ background: `linear-gradient(135deg,${ec},${ec}99)`, borderRadius: 20, padding: 20, color: "#fff", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
          <button onClick={() => onEditar(escuela)} style={{ background: "rgba(255,255,255,.2)", border: "none", borderRadius: 10, padding: "8px 14px", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>✏️ Editar</button>
        </div>
      </div>
      ...
      {(escuela.director || escuela.secretaria || (escuela.eoe || []).length > 0) && (
        <Card>
          ...
          {(escuela.eoe || []).map((m, i) => (
            <div key={i} style={{ padding: "10px 0", borderBottom: i < (escuela.eoe.length - 1) ? `1px solid ${BD}` : "none" }}>
      ...
      {docsEsc.length > 0 && (
        <Card>
          ...
          {docsEsc.map((d, i) => (
            <div key={d.id} style={{ padding: "10px 0", borderBottom: i < docsEsc.length - 1 ? `1px solid ${BD}` : "none" }}>
      ...
      <Card>
        ...
        {alusAct.length === 0
          ? <div style={{ color: GL, fontSize: 13, fontStyle: "italic" }}>Sin alumnos activos en esta escuela</div>
          : alusAct.map((a, i) => (
              <div key={a.id} style={{ padding: "8px 0", borderBottom: i < alusAct.length - 1 ? `1px solid ${BD}` : "none" }}>
...
function SecEscuelas({ escuelas, alumnos, docentes, onSave, onDelete, onToggleActivo }) {
  ...
  const initForm = (e = null) => setForm(e ? { ...e } : {
  ...
  if (ficha) return <FichaEscuela escuela={ficha} alumnos={alumnos} docentes={docentes} onEditar={e => { setFicha(null); initForm(e); }} onBack={() => setFicha(null)} />;
    ...
    <div>
      ...
      {form ? (
        <div style={{ background: "#f8fafc", border: `2px solid ${form.color || G}`, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 12 }}>{form.id && escuelas.find(e => e.id === form.id) ? "Editar escuela" : "Nueva escuela"}</div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <select value={form.nivel || "Primaria"} onChange={e => set("nivel", e.target.value)} style={{ width: "100%", border: `1.5px solid ${BD}`, borderRadius: 10, padding: "10px 14px", fontSize: 14, fontFamily: "inherit", background: "#f8fafc", boxSizing: "border-box" }}>
              {["Inicial", "Primaria", "Secundaria", "Especial"].map(n => <option key={n} value={n}>{n}</option>)}
            </select>
          </div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: GR, textTransform: "uppercase", marginBottom: 8 }}>Color institucional</div>
          ...
          <div style={{ background: "#fff", border: `1.5px solid ${BD}`, borderRadius: 12, padding: "12px 14px", marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: TX }}>🔬 Equipo de Orientación Escolar (EOE)</div>
            ...
            {(form.eoe || []).length === 0
              ? <div style={{ fontSize: 12, color: GL, fontStyle: "italic" }}>Sin integrantes del EOE cargados</div>
              : (form.eoe || []).map((m, i) => (
                  <div key={i} style={{ borderTop: `1px solid ${BD}`, paddingTop: 10, marginTop: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ fontWeight: 600, fontSize: 13, color: TX }}>Integrante {i + 1}</div>
                    ...
                    <Fld label="Rol" value={m.rol} onChange={v => setEoe(i, "rol", v)} placeholder="Orientadora, Psicóloga..." />
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <Btn onClick={() => { onSave(form); setForm(null); }} color={form.color || G} full disabled={!form.nombre}>Guardar</Btn>
          </div>
        </div>
      ) : (
        <button onClick={() => initForm()} style={{ width: "100%", padding: 12, borderRadius: 14, border: `2px dashed ${G}`, background: "#f0fdf4", color: G, fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit", marginBottom: 12 }}>+ Nueva escuela</button>
      )}
      ...
      {activas.length === 0
        ...
        : activas.map(e => {
            ...
            return (
              <Card key={e.id} onClick={() => !form && setFicha(e)} sx={{ borderLeft: `4px solid ${inact ? "#cbd5e0" : e.color || G}`, opacity: inact ? .7 : 1, cursor: "pointer" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: GL, marginTop: 2 }}>
                      👤 {aluCount} alumnos
                      {e.director ? ` · 👤 ${e.director}` : ""}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <button onClick={ev => { ev.stopPropagation(); initForm(e); }} style={{ background: (e.color || G) + "15", border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: e.color || G, fontWeight: 700, fontSize: 12, fontFamily: "inherit" }}>Editar</button>
                    <button onClick={ev => { ev.stopPropagation(); setConf({ msg: `¿${inact ? "Reactivar" : "Archivar"} ${e.nombre}?`, ok: () => { onToggleActivo(e.id); setConf(null); } }); }} style={{ background: "#f1f5f9", border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: GR, fontWeight: 700, fontSize: 12, fontFamily: "inherit" }}>{inact ? "▶" : "⏸"}</button>
...
// ── SecDocentes ───────────────────────────────────────────────
function SecDocentes({ docentes, escuelas, onSave, onDelete }) {
  const [form, setForm] = useState(null);
  const [conf, setConf] = useState(null);
  ...
  const filtrados = activos.filter(d => {
    ...
    if (busq) {
      ...
      return d.nombre.toLowerCase().includes(q) || d.materia.toLowerCase().includes(q);
    }
    return true;
  });

  const initForm = (d = null) => setForm(d ? { ...d } : { id: uid(), nombre: "", materia: "", escuelaId: "", telefono: "", mail: "", eliminado: false });
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));
  ...
  return (
    <div>
      ...
      {form ? (
        ...
        <button onClick={() => initForm()} style={{ width: "100%", padding: 12, borderRadius: 14, border: `2px dashed ${G}`, background: "#f0fdf4", color: G, fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit", marginBottom: 12 }}>+ Nuevo docente</button>
      )}

      {/* Búsqueda y filtro por escuela */}
      {!form && activos.length > 0 && <>
        <input value={busq} onChange={e => setBusq(e.target.value)} placeholder="🔍 Buscar por nombre o materia..." style={{ width: "100%", border: `1.5px solid ${BD}`, borderRadius: 10, padding: "9px 14px", fontSize: 13, fontFamily: "inherit", boxSizing: "border-box", marginBottom: 8 }} />
        {escuelas.filter(e => !e.eliminado).length > 1 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12, overflowX: "auto", paddingBottom: 2 }}>
            ...
            {escuelas.filter(e => !e.eliminado).map(e => (
              <button key={e.id} onClick={() => setFilEsc(filEsc === e.id ? "" : e.id)} style={{ padding: "6px 12px", borderRadius: 20, border: "2px solid", fontFamily: "inherit", fontWeight: 700, fontSize: 12, cursor: "pointer", borderColor: filEsc === e.id ? (e.color || G) : BD, background: filEsc === e.id ? (e.color || G) : "#fff", color: filEsc === e.id ? "#fff" : "#475569", whiteSpace: "nowrap" }}>{e.nombre}</button>
            ))}
          </div>
        )}
      </>}
      ...
      {filtrados.length === 0
        ? <Card sx={{ textAlign: "center", padding: 32, color: GR }}>{activos.length === 0 ? "Sin docentes cargados." : "Sin resultados para esa búsqueda."}</Card>
        : filtrados.map(d => {
            const esc = escuelas.find(e => e.id === d.escuelaId);
            return (
              <Card key={d.id} sx={{ borderLeft: `4px solid ${esc?.color || G}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                    <button onClick={() => initForm(d)} style={{ background: "#f1f5f9", border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: GR, fontWeight: 700, fontSize: 12, fontFamily: "inherit" }}>Editar</button>
                    <button onClick={() => setConf({ msg: `¿Eliminar a ${d.nombre}?`, ok: () => { onDelete(d.id); setConf(null); } })} style={{ background: "#fef2f2", border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: "#dc2626", fontWeight: 700, fontSize: 12, fontFamily: "inherit" }}>🗑</button>
                  </div>
                </div>
              </Card>
            );
...
function SecProfesionales({ pros, onSave, onDelete }) {
  ...
  const initForm = (p = null) => setForm(p ? { ...p } : { id: uid(), nombre: "", rol: "", telefono: "", mail: "", eliminado: false });
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));
    ...
    <div>
      ...
      {form ? (
        <div style={{ background: "#f8fafc", border: `2px solid #7c3aed`, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          ...
          <Fld label="Rol / Especialidad" value={form.rol} onChange={v => set("rol", v)} placeholder="Psicopedagoga, Fonoaudióloga..." />
          <Fld label="Teléfono" value={form.telefono} onChange={v => set("telefono", v)} placeholder="11-xxxx-xxxx" />
        ...
        <button onClick={() => initForm()} style={{ width: "100%", padding: 12, borderRadius: 14, border: "2px dashed #7c3aed", background: "#faf5ff", color: "#7c3aed", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit", marginBottom: 12 }}>+ Nuevo profesional externo</button>
      )}
        ...
        : activos.map(p => (
            <Card key={p.id} sx={{ borderLeft: "4px solid #7c3aed" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <button onClick={() => initForm(p)} style={{ background: "#f5f3ff", border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: "#7c3aed", fontWeight: 700, fontSize: 12, fontFamily: "inherit" }}>Editar</button>
  ...
  return (
    <div>
      ...
      <div style={{ padding: "16px 16px 80px" }}>
        {seccion === "escuelas" && <SecEscuelas escuelas={escuelas} alumnos={alumnos} docentes={docentes} onSave={saveEsc} onDelete={delEsc} onToggleActivo={toggleActivoEsc} archivarAlumnosEsc={archivarAlumnosEsc} />}
        {seccion === "docentes" && <SecDocentes docentes={docentes} escuelas={escuelas} onSave={saveDoc} onDelete={delDoc} />}
